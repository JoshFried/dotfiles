...
function M.on_attach(client, buffer)
    ...
    self:map("<leader>rl", function()
        require("telescope.builtin").registers()
    end, { remap = true, desc = " and paste register in insert mode", silent = false })


    self:map("<leader>yf", ":%y<cr>", { desc = "yank current file to the clipboard buffer" })
end
...
function M.rename()
    ...
    for _, client in ipairs(vim.lsp.get_clients()) do
        if vim.tbl_contains(fallback_clients, client.name) then
            vim.lsp.buf.rename()
            return
        end
    end
